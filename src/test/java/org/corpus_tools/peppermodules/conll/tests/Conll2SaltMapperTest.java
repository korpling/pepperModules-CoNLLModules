...
public class Conll2SaltMapperTest {
	...
	private Conll2SaltMapper getFixture() {
		return fixture;
	}

	private void setFixture(Conll2SaltMapper fixture) {
	...
	private String[] tokenFormsExpected = { "Die", "Jugendlichen", "in", "Zossen", "wollen", "ein", "Musikcafe", "." };

	private int rootTokenIndex = 4; // index of root token (has no incoming
									// pointing relations)

	private String[][] tokenAnnotationNamesExpected = { { "CPOSTAG", "POSTAG", "case", "num", "gender" }, // "Die"
	...
	private String[][] tokenAnnotationValueStringsExpected = { { "ART", "ART", "Nom", "Pl", "*" }, { "N", "NN", "Nom", "Pl", "*" }, { "PREP", "APPR" }, { "N", "NE", "Dat", "Sg", "Neut" }, { "V", "VMFIN", "3", "Pl", "Pres", "Ind" }, { "ART", "ART", "Acc", "Sg", "Neut" }, { "N", "NN", "Acc", "Sg", "Neut" }, { "$.", "$." }, };

	private String pointingRelationAnnotationNameExpected = "deprel";

	private String[] pointingRelationAnnotationValueStringsExpected = { "DET", "P_DET", "SUBJ", "P_SUBJ", "PP", "P_PP", "PN", "P_PN",
			// here is the root node´s position
			"DET", "P_DET", "OBJA", "P_OBJA", "-PUNCT-", "P_-PUNCT-" };
	...
	private int[] sourceNodeIDExpected = { 2, 2, 5, 5, 5, 5, 3, 3,
			// here is the root node´s position
			7, 7, 5, 5, 7, 7 };
	...
	private void printPointingRelations(SDocumentGraph docGraph) {
		for (SPointingRelation rel : docGraph.getPointingRelations()) {
			...
			System.out.println(String.format("%s --(%s:%s)--> %s", getTokenForm(src), rel.getAnnotations().iterator().next().getName(), rel.getAnnotations().iterator().next().getValue(), getTokenForm(trg)));
		}
	}
  ...
  public void testNoSentenceAnnotation()
  {
	...
	// assertEquals(8,
	// getFixture().getDocument().getDocumentGraph().getTokens().size());
	//
	// //iterate over tokens
	// int tokenIndex=0;
	// for (SToken token :
	// getFixture().getDocument().getDocumentGraph().getTokens()) {
	// // check sDocumentGraph
	// assertEquals(getFixture().getDocument().getDocumentGraph(),
	// token.getDocumentGraph());
	//
	// //check annotations
	// int annotationIndex=0;
	// for (SAnnotation annotation : token.getAnnotations()) {
	// //check annotations
	// assertEquals(tokenAnnotationNamesExpected[tokenIndex][annotationIndex],
	// annotation.getName());
	// assertEquals(tokenAnnotationValueStringsExpected[tokenIndex][annotationIndex],
	...
	// assertEquals(token,
	// getFixture().getDocument().getDocumentGraph().getTextualRelations().get(tokenIndex).getSource());
	//
	// //check associated spanning relation (linear order -> same index as
	// token)
	// assertEquals(token,
	// getFixture().getDocument().getDocumentGraph().getSpanningRelations().get(tokenIndex).getTarget());
	//
	// //check associated pointing relations; tests depend on properties...
	//// boolean projectivity =
	// getFixture().getProperties().getProperty(CoNLLImporterProperties.PROP_CONSIDER_PROJECTIVITY,
	// "NO").equals("YES");
	// boolean projectivity =
	// getFixture().getProperties().getProperty(CoNLLImporterProperties.PROP_CONSIDER_PROJECTIVITY).getType().equals("YES");
	// int proFactor = 2; //proFactor is the factor that tokenIndex has to be
	// multiplied with if projectivity is considered (double number of
	// pointingRels)
	// if (!projectivity) {
	// proFactor = 1;
	// }
	...
	// }
	// assertEquals(token,
	// getFixture().getDocument().getDocumentGraph().getPointingRelations().get(pointingRelationIndex).getTarget());
	// if (projectivity) {
	// assertEquals(token,
	// getFixture().getDocument().getDocumentGraph().getPointingRelations().get(pointingRelationIndex+1).getTarget());
	// }
	// }
	// tokenIndex++;
	// }
	...
	public final void testTextualRelations() {
		...
		// getFixture().getDocumentGraph().getTextualDSs().get(0);
		//
		// int trIndex = 0;
		// for (STextualRelation tr :
		// getFixture().getDocumentGraph().getTextualRelations()) {
		// //check sDocumentGraph
		// assertEquals(getFixture().getDocumentGraph(), tr.getDocumentGraph());
		// //check associated source token (linear order -> same index as
		// textualRelation)
		// assertEquals(getFixture().getDocumentGraph().getTokens().get(trIndex),
		// tr.getSource());
		// //check associated target node
		// assertEquals(textualDS, tr.getTarget());
		// //check start and end via string comparison in textualDS
		// assertEquals(tokenFormsExpected[trIndex],
		// textualDS.getText().substring(tr.getStart(), tr.getEnd()));
		// trIndex++;
		// };
	}
	...
	public final void testSpans() {
		...
		// //check sDocumentGraph
		// assertEquals(getFixture().getDocumentGraph(),
		// span.getDocumentGraph());
		// //check annotation
		// assertEquals(spanNameExpected,
		// span.getAnnotations().get(0).getName());
		// assertEquals(spanValueStringExpected,
		// span.getAnnotations().get(0).getValueString());
		// //check associated spanning relation (linear order -> same index as
		// span)
		// assertEquals(getFixture().getDocumentGraph().getSpanningRelations().get(spanIndex).getSource(),
		// span);
		// spanIndex++;
		// };
	}
	...
	public final void testSpanningRelations() {
		...
		//
		// int spanRelIndex = 0;
		// for (SSpanningRelation spanRel :
		// getFixture().getDocumentGraph().getSpanningRelations()) {
		// //check sDocumentGraph
		// assertEquals(getFixture().getDocumentGraph(),
		// spanRel.getDocumentGraph());
		// //check associated span (linear order -> same index as spanning
		// relation)
		// assertEquals(getFixture().getDocumentGraph().getSpans().get(spanRelIndex),
		// spanRel.getSource());
		// //check associated token (linear order -> same index as spanning
		// relation)
		// assertEquals(getFixture().getDocumentGraph().getTokens().get(spanRelIndex),
		// spanRel.getTarget());
		// spanRelIndex++;
		// }
	}
	...
	public final void testPointingRelations() {
		...
		//
		// boolean projectivity =
		// getFixture().getProperties().getProperty(Conll2SaltMapper.PROPERTYKEY_PROJECTIVITY,
		// Conll2SaltMapper.FALSE).equals(Conll2SaltMapper.TRUE);
		// int proFactor = 1;
		...
		// }
		//
		// boolean projectiveModeIsType =
		// getFixture().getProperties().getProperty(Conll2SaltMapper.PROPERTYKEY_PROJECTIVEMODE,
		// Conll2SaltMapper.TYPE).equals(Conll2SaltMapper.TYPE);
		//
		// int pointRelIndex = 0;
		// for (SPointingRelation pointRel :
		// getFixture().getDocumentGraph().getPointingRelations()) {
		// //check sDocumentGraph
		// assertEquals(getFixture().getDocumentGraph(),
		// pointRel.getDocumentGraph());
		// //check annotation and stype
		// assertEquals(pointingRelationAnnotationNameExpected,
		// pointRel.getAnnotations().get(0).getName());
		// assertEquals(pointingRelationAnnotationValueStringsExpected[pointRelIndex*proFactor],
		// pointRel.getAnnotations().get(0).getValueString());
		// if (projectivity) {
		// // is index even or odd? pointing relation is "normal" if true,
		// projective if false
